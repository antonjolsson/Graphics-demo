...
// Note that this implementation is little overcomplicated because we are saving/setting up/restoring every OpenGL state explicitly, in order to be able to run within any OpenGL inputHandler that doesn't do so.
// If text or lines are blurry when integrating ImGui in your inputHandler: in your Render function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f)
